...
public class AvionPasajeroController {
    ...
    public String updateAvion(@Valid @ModelAttribute("avion") Avion avion,
        ...
        try {
            ...
            avionRepository.save(avion);
            logger.info("Aviób con ID {} actualizado con éxito.",
                    avion.getId());
        } catch (Exception e) {
    ...
    public String searchPasajero(@RequestParam("pasajeroSearch") String
                                        pasajeroSearch, @RequestParam("ticketId") Long avionId, Model model) {
    ...
    public String addPasajeroToAvion(@RequestParam("aviontId") Long avionId,
                                     @RequestParam("pasajeroId") Long pasajeroId, RedirectAttributes
                                             redirectAttributes, Locale locale) {
        logger.info("Añadiendo pasajero con ID {} al avion con ID {}", pasajeroId, avionId);
        try {
            Optional<Avion> avionOpt = avionRepository.findById(avionId);
    ...
    public String addNewPasajeroToAvion(@RequestParam("avionId") Long
        ...
        try {
            ...
            logger.error("Error al añadir el nuevo pasajero al avión: {}",
                    e.getMessage());
            redirectAttributes.addFlashAttribute("errorMessage", "Error al avión el nuevo pasajero.");
        }
        return "redirect:/aviones/detail?id=" + avionId;
    }
